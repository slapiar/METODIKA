...
```text
GATE=CLOSED
BLOKUJÚCI_BOD=4. Potrebné prístupy prakticky overené
CHÝBAJÚCI_DÔKAZ=spätné načítanie tohto prvého povoleného zápisu zo vzdialenej vetvy main
POVOLENÝ_ĎALŠÍ_ÚKON=iba spätné načítanie tohto INI a doplnenie dôkazu bodu 4
```
...
| 3. Vetva a HEAD overené | ÁNO | Autoritatívna vetva `main` a aktuálna história | HEAD pred INI `d418e72c162bde324af7546c937af979bd75182e` | HEAD po vytvorení INI |
| 4. Potrebné prístupy prakticky overené | NEOVERENÉ | Čítanie repozitára a administrátorské oprávnenia sú potvrdené | repozitár hlási `admin=true`, `push=true`; úspešné vzdialené čítanie | úspešný zápis a jeho vzdialený read-back |
| 5. Prostredie prakticky overené | ÁNO | Pre tento úkon je potrebné iba vzdialené GitHub prostredie; databáza, runtime a produkcia nie sú predmetom zásahu | aktuálne vzdialené čítanie repozitára je funkčné | nič |
